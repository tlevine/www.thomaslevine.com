...
#### Approach 1a: Combining posterior regression coefficients
...
variable for its value in the public scenario and one for the corresponding
private scenario; this halved the number of rows. I checked simply whether
posture dirtiness was the same between the corresponding public and private
conditions.

male | defecate | cleanliness | p.different
--- | --- | --- | --- | --- | --- | --- | ---
0 | 0 | unspecified | 0.49515
0 | 0 | clean | 0.34951
0 | 0 | dirty | 0.03883
0 | 1 | unspecified | 0.17476
0 | 1 | clean | 0.22330
0 | 1 | dirty | 0.06796
1 | 0 | unspecified | 0.04286
1 | 0 | clean | 0.01429
1 | 0 | dirty | 0.04286
1 | 1 | unspecified | 0.08571
1 | 1 | clean | 0.01429
1 | 1 | dirty | 0.12857


This doesn't tell us the sign, but it does suggest that there is indeed a
difference for some of the scenarios. Most of the time, people don't switch
between clean and dirty postures when the scenario switches between public
and private. However, the chance of switching is much higher for females
than males, and it is almost 50% for one of the scenarios (female urinating
at a toilet of unspecified cleanliness).

These simple cross-tabulations don't give us an idea of how precise they
are; it could be that I would get very different numbers if I asked 100
other people. In order to check that, I ran logistic regression on the
formula, again with a participant random effect.

    I(public == private) ~ male * defecate * cleanliness

I then combined the posterior distributions of the regression coefficients
and plotted XXX the resulting probability distributions for the
probabilities that one will switch between dirty and clean postures when
he or she switches between the corresponding public and private scenarios;
these are like the probabilities that are cross-tabulated, except that
they are distributions, so we have an idea of the precision of these estimates.

The widest of these distributions is about one-fifth wide; said more technically,
its range is one-fifth. This is precise enough that I'd say that the effect of
sex is significant; sex probably does have a real impact on whether
someone switches postures when he or she changes from a private toilet to an
otherwise similar public toilet.

#### Agreement of approaches

These various approaches provide similar answers.

The first approach of using private versus public as an independent variable
suggested that people are more likely to use a dirty posture at a public toilet
than at an otherwise similar private toilet. The change in likelihood was
blown out of proportion, and I don't really know why, but the change was
consistently in the direction of more dirty postures at public toilets.

The second approach of using a difference in posture between the public and
private scenarios showed more simply that the posture indeed seems to change
between private and public scenarios when everything else is controlled.
It gives us no indication of the direction of the change (that is, whether
public toilets encourage clean postures or dirty postures), but the numbers
seemed more reasonable. It also pointed out differing responses of males
and females.

Based on the vague agreement of these three thought processes, it appears
that public toilets encourage dirty postures for reasons other than public
toilets' chance of being dirty, particularly among females.

#### Validity of questionnaire responses

A note of caution about my attempt to separate effects of toilet
